...
**采用方案：** 方案 C - 保持现状 + 面包屑优化

**理由：**
- 当前 URL 结构语义清晰，符合 Next.js App Router 设计
- Tab 式会破坏浏览器前进/后退体验
- 侧边栏在移动端占用空间，实现复杂

**已完成：**
- 创建 `components/ui/breadcrumb.tsx` 面包屑组件
- 支持 Home 图标、可点击链接、当前页高亮
- 已添加面包屑的页面：
  - `/settings/expenses` - 消费配置
  - `/settings/expenses/recurring` - 固定支出
  - `/settings/advanced` - 高级配置
  - `/settings/advanced/cron` - 定时任务
...
- [x] 金额输入：移除默认 "0"，使用 placeholder
- [x] 删除操作：添加确认弹窗（已存在于 GroupedList.tsx）
- [ ] 批量操作：添加批量删除/分类功能（待设计，需考虑安全性 5.1）

**已完成：**
- app/add/page.tsx 金额输入默认值从 '0' 改为 ''
- 清空按钮和表单重置也使用空字符串
- onBlur 只在有效金额时格式化
- 验证提示只在用户输入了无效值时显示
