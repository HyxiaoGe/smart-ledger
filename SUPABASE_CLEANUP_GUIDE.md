...
所有 Supabase 依赖已成功移除，项目现在完全使用 **Prisma + PostgreSQL**。

详细执行记录请查看: [CLEANUP_PLAN.md](./CLEANUP_PLAN.md)

---

## 一、当前依赖概览 (历史记录)

~~项目目前采用 **双数据库架构**（Supabase + Prisma），通过环境变量 `USE_PRISMA=true` 控制切换。~~ **已清理完成**
