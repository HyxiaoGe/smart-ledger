    ...
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      {showSuggestions && !disabled && (
        <div className="absolute top-full left-0 right-0 z-50 mt-1 max-h-60 overflow-y-auto rounded-md border border-gray-200 bg-white shadow-lg">
          {isLoading ? (
            <div className="p-3 text-center text-sm text-muted-foreground">�����С�</div>
            ...
            suggestions.map((suggestion, index) => (
              ...
              >
                ...
                <span className="flex items-center gap-2 text-xs text-muted-foreground">
                  {suggestion.usage_count > 5 ? <Badge variant="outline">����</Badge> : null}
                  <span>{suggestion.usage_count} ��</span>
                </span>
              </button>
            ))
          ) : (
            <div className="p-3 text-center text-sm text-muted-foreground">û��ƥ��ĳ��ñ�ע</div>
          )}
        </div>
      )}
    </div>
