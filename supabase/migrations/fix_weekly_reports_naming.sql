...
BEGIN
  ...
  -- 统计支付方式 (注意字段名改为 payment_method_stats)
...
-- 5. 创建访问策略
CREATE POLICY IF NOT EXISTS "Users can view all weekly reports"
  ON weekly_reports FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "System can insert weekly reports"
  ON weekly_reports FOR INSERT WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "System can update weekly reports"
